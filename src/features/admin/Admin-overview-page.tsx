    ...
    return (
        <div>
            <div className="mb-8 stagger-1">
                <h1 className="font-display text-2xl font-bold text-brand-900 mb-1">Overview</h1>
                <p className="text-brand-900/60 text-sm">Platform health at a glance.</p>
            </div>
                ...
                <>
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                        {stats.map(({ label, value, icon: Icon, color, bg }, i) => (
                            <div key={label} className={`bg-surface-50 border border-brand-900/10 rounded-2xl p-5 stat-card stagger-${(i % 5) + 1}`}>
                                ...
                                <p className="text-brand-900/60 text-xs mt-1">{label}</p>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="grid lg:grid-cols-2 gap-4">
                        <div className="bg-surface-50 border border-brand-900/10 rounded-2xl p-6 stagger-3">
                            <div className="flex items-center gap-2 mb-4">
                                ...
                                <h2 className="font-display font-semibold text-brand-900 text-sm">User health</h2>
                            </div>
                            <div className="flex flex-col gap-2">
                                <div className="flex justify-between text-sm">
                                    <span className="text-brand-900/60">Active</span>
                                    <span className="text-sage-500 font-mono">{users.length - blocked}</span>
                                </div>
                                <div className="flex justify-between text-sm">
                                    <span className="text-brand-900/60">Blocked</span>
                                    <span className="text-red-500 font-mono">{blocked}</span>
                                </div>
                                {users.length > 0 && (
                                    <div className="mt-3 h-2 bg-brand-900/5 rounded-full overflow-hidden">
                                        <div
                                            className="h-full bg-sage-500/60 rounded-full transition-all duration-1000 ease-out"
                        ...
                        <div className="bg-surface-50 border border-brand-900/10 rounded-2xl p-6 stagger-4">
                            <div className="flex items-center gap-2 mb-4">
                                ...
                                <h2 className="font-display font-semibold text-brand-900 text-sm">Therapist pipeline</h2>
                            </div>
                            <div className="flex flex-col gap-2">
                                {[
                                    { label: "Pending", count: pending, color: "text-yellow-600" },
                                    { label: "Approved", count: approved, color: "text-sage-500" },
                                    { label: "Rejected", count: therapists.filter((t) => t.status === "rejected").length, color: "text-red-500" },
